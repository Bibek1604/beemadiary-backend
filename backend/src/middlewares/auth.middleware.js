...
// When AUTH_BYPASS=true (and NODE_ENV is not "production"), protected routes
// skip token verification and run as a default agent/admin so you don't have
// to log in while developing locally. This is HARD-DISABLED in production.
// Set DEV_AGENT_ID in .env to pin a specific agent; otherwise the first active
// agent/admin is used. NEVER enable this on a server reachable by others.
// ---------------------------------------------------------------------------
const AUTH_BYPASS =
...
if (AUTH_BYPASS) {
  ...
  console.warn(
    "⚠️  AUTH_BYPASS is ON — all protected routes run without login (dev only). Disable before deploying."
  );
}
...
async function resolveDevUser(kind) {
  try {
    if (kind === "AGENT") {
      if (_devAgent) return _devAgent;
      const explicitId = process.env.DEV_AGENT_ID;
      const agent = explicitId
        ? await prisma.agent.findFirst({ where: { id: explicitId, deleted_at: null }, select: { id: true, email: true, company_id: true } })
        : await prisma.agent.findFirst({ where: { deleted_at: null, status: "ACTIVE" }, select: { id: true, email: true, company_id: true } });
      if (!agent) return null;
      _devAgent = { id: agent.id, email: agent.email, role: "AGENT", type: "AGENT", company_id: agent.company_id };
      return _devAgent;
    }
    if (_devAdmin) return _devAdmin;
    const admin = await prisma.admin.findFirst({ where: { deleted_at: null, status: "ACTIVE" }, select: { id: true, email: true } });
    if (!admin) return null;
    _devAdmin = { id: admin.id, email: admin.email, role: "ADMIN", type: "ADMIN" };
    return _devAdmin;
  } catch {
    return null;
  }
...
const authenticate = async (req, res, next) => {
  try {
    if (AUTH_BYPASS) {
      const devUser = await resolveDevUser("AGENT");
      if (devUser) {
        req.user = devUser;
...
const authenticateAdmin = async (req, res, next) => {
  try {
    if (AUTH_BYPASS) {
      const devUser = await resolveDevUser("ADMIN");
      if (devUser) {
        req.user = devUser;
        req.token = "dev-bypass";
        return next();
